...
void calculate()
{
  ...
  if (countAverage == numAverage) {
    //sort
    if (countSilent == 0) {
      lzSum += myLidarLite.distance(true);
      lzSum/=100.0;
    }
...
//    
    ...
    if (countSilent == 0)
    {
    transData2 = "H voile: " + String(lzSum,0) + "cm      " ;
    lcd.setCursor(0, 1);
    lcd.print(transData2);
    transData2 = "dv " + String(lzSum,0) ;
    }
    Serial.println(transData2);
    countSilent++;
    if (countSilent == 3) countSilent = 0;
    lzSum = 0.0;
    
  } else if (countSilent == 0) lzSum += myLidarLite.distance(false);
